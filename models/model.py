...
class DecoratorModel:
    ...
    def load_from_file(cls, path, mode="train"):
        ...
        # vocabulary = data['model']['vocabulary']
        # print('vocabulary:', vocabulary)
        # print('vars(vocabulary):', vars(vocabulary))
        # print('dir(vocabulary):', dir(vocabulary))
    ...
    def sample_decorations(self, scaffold_seqs, scaffold_seq_lengths):
        ...
        input_vector = torch.full((batch_size, 1), self.vocabulary.decoration_vocabulary["^"], dtype=torch.long, device=device)
        seq_lengths = torch.ones(batch_size, device='cpu')

        scaffold_seqs = scaffold_seqs.to(device)
        scaffold_seq_lengths = scaffold_seq_lengths.cpu()
        ...
        for _ in range(self.max_sequence_length - 1):
            logits, hidden_states, _ = self.network.forward_decoder(input_vector, seq_lengths, encoder_padded_seqs, hidden_states)  # (batch, 1, voc)
        ...
        decoration_smiles = [self.vocabulary.decode_decoration(seq) for seq in torch.cat(sequences, 1).data.cpu().numpy()]#voc cat
