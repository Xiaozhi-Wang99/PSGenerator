...
class TrainModel(Action):
    ...
    def run(self):
        ...
        for epoch, training_set in zip(range(1, self.epochs + 1), self.training_sets):
            ...
            print('post_epoch_status:', post_epoch_status)
            if not post_epoch_status: #如果post_epoch_status=False，会提前终止
                break
...
class SampleModel(Action):
    ...
    def run(self, scaffold_list):
        ...
        """
        device = next(self.model.network.parameters()).device #add
        dataset = md.Dataset(scaffold_list, self.model.vocabulary.scaffold_vocabulary, self.model.vocabulary.scaffold_tokenizer)
        dataloader = tud.DataLoader(dataset, batch_size=self.batch_size, shuffle=False, collate_fn=md.Dataset.collate_fn)

        for batch in dataloader:
            scaffold_seqs, scaffold_seq_lengths = batch
            scaffold_seqs = scaffold_seqs.to(device)
            scaffold_seq_lengths = scaffold_seq_lengths.to('cpu')
