...
conf = SparkConf()\
    .setAppName("sample_scaffolds")\
    .setMaster("local[*]")\
    .set("spark.executor.heartbeatInterval", "120s")\
    .set("spark.network.timeout", "1600s")\
...
class SampleScaffolds(ma.Action):
    ...
    def run(self, initial_scaffolds):
        ...
        while scaffolds_df.count() > 0:
            ...
            scaffolds_df = scaffolds_df.withColumn("randomized_scaffold", randomized_scaffold_udf("smiles"))\
                .select("smiles", "scaffold", "decorations",psf.explode("randomized_scaffold").alias("randomized_scaffold"))\
            ...
            self._log("info", "Generated %d randomized SMILES from %d scaffolds.",
                      scaffolds_df.count(), scaffolds_df.select("smiles").distinct().count())#Remove duplicates
            ...
            if joined_df.count() > 0:
                self._log("info", "Joined %d -> %d (valid) sampled scaffolds",
                          scaffolds_df.count(), joined_df.count())

            scaffolds_df = joined_df.join(results_df, on="smiles", how="left_anti")\
                .select("id", "smiles", "scaffold", "decorations")\
                .where("smiles LIKE '%*%'")
            self._log("info", "Obtained %d scaffolds for next iteration.", scaffolds_df.count())

            results_df = joined_df.persist()
            i += 1

        return results_df

    def _initialize_results(self, scaffolds):
        data = [ps.Row(id={}, smiles=scaffold, scaffold=scaffold, decorations={}) for scaffold in scaffolds]
        data_schema = pst.StructType([
            pst.StructField("id", pst.StringType()),
    ...
    def _sample_and_write_scaffolds_to_disk(self, scaffolds, total_scaffolds):
        ...
        if os.path.getsize(self._tmp_path("sampled_decorations")) == 0:
            self._log("error", "No data was written to sampled_decorations file.")
            raise RuntimeError("Sampled file is empty after sampling.")
    ...
    def _join_results(self, scaffolds_df):
        ...
        if sampled_rdd.isEmpty():
            self._log("error", "The sampled RDD is empty.")
    ...
    def _join_results_multi(self, scaffolds_df, sampled_df):
        ...
        sampled_df_norm = sampled_df.withColumn("id", trim(lower(col("id"))).cast("string"))
        scaffolds_df_norm = scaffolds_df.withColumn("id", trim(lower(col("id"))).cast("string"))

        joined_result = sampled_df_norm.join(scaffolds_df_norm, on="id", how="left")\
        ...
        print(f"joined_result Total {joined_result.count()} row，{len(joined_result.columns)} column")
...
def parse_args():
    ...
    parser.add_argument("--output-prefix", "-op", type=str, default="Gen_guacamol_PDT")
    parser.add_argument("--device", type=str, default="cpu", help="Device to use: cuda or cpu")

    return parser.parse_args()


def process_smiles_csv(file_path, output_path, figure_path, output_path4):
    df = pd.read_csv(file_path)
    df_non_null = df.dropna(subset=['smiles'], axis=0)

    count_series = df_non_null['smiles'].value_counts()
    df_unique = df_non_null.drop_duplicates(subset=['smiles']).copy()
    df_unique['count'] = df_unique['smiles'].map(count_series)
    df_unique = df_unique.sort_values(by='count', ascending=False)

    df_unique.to_csv(output_path, index=False)
    df_non_null.to_csv(output_path4, index=False)
...
def main():
    ...
    print(f"Subprocess is using device: {device}")
    ...
    output_path = os.path.join(args.output_dir, f"{args.output_prefix}_repeat_ori.csv")
    output_path2 = os.path.join(args.output_dir, f"{args.output_prefix}_unique.csv")
    output_path3 = os.path.join(args.output_dir, f"{args.output_prefix}_unique_top10.png")
    output_path4 = os.path.join(args.output_dir, f"{args.output_prefix}_repeat_dropna.csv")

    if args.output_format == "parquet":
        results_df.write.parquet(output_path)
    else:
        results_df.toPandas().to_csv(output_path)

    process_smiles_csv(output_path, output_path2, output_path3, output_path4)
